...
public class ZRClusterClient extends ZRCluster {
	...
	public void setApiSettings(String packageName, boolean open, List<String> authoritys) {
		...
		zker.set(ZR_API_SETTINGS, packageName, JsonUtil.objToJson(e));
	}
	...
	public void setApiVersionSettings(String methodName, String version, boolean open, int topology,
		...
		zker.set(ZR_API_VERSION_SETTINGS, fullName, JsonUtil.objToJson(e));
	}
	...
	public ZRMachineInfoVo getMachineInfo(String ip) {
		...
		if (value == null)
			return null;
		ZRMachineInfoVo info = JsonUtil.jsonToObj(value, ZRMachineInfoVo.class);
	...
	public List<ZRMachineInfoVo> getMachineInfo() {
		...
		for (String e : values.values()) {
			ZRMachineInfoVo info = JsonUtil.jsonToObj(e, ZRMachineInfoVo.class);
			info.setOpen(true);
	...
	public ZRServerInfoVo getServerInfo(String serverId) {
		...
		if (value == null)
			return null;
		ZRServerInfoVo info = JsonUtil.jsonToObj(value, ZRServerInfoVo.class);
	...
	public List<ZRServerInfoVo> getServerInfoByMachineIp(String ip) {
		...
		for (String e : values.values()) {
			ZRServerInfoVo info = JsonUtil.jsonToObj(e, ZRServerInfoVo.class);
	...
	public ZRServiceInfoVo getServiceInfo(String serviceId) {
		...
		if (value == null)
			return null;
		ZRServiceInfoVo info = JsonUtil.jsonToObj(value, ZRServiceInfoVo.class);
	...
	public List<ZRServiceInfoVo> getServiceInfoByServerId(String serverId) {
		...
		for (String e : values.values()) {
			ZRServiceInfoVo info = JsonUtil.jsonToObj(e, ZRServiceInfoVo.class);
	...
	public List<ZRModuleVo> getApis() {
		...
		for (String e : values.values()) {
			ZRApiInfoVo info = JsonUtil.jsonToObj(e, ZRApiInfoVo.class);
			info.setOpen(true);
		...
		for (Entry<String, String> e : values.entrySet()) {
			...
			if (info != null) {
				ZRApiVersionSettings settings = JsonUtil.jsonToObj(e.getValue(), ZRApiVersionSettings.class);
	...
	public List<ZRModuleVo> getApisByServiceId(String serviceId) {
		...
		ZRServiceInfo serviceinfo = JsonUtil.jsonToObj(value, ZRServiceInfo.class);
	...
	private List<ZRModuleVo> getModules(Map<String, ZRApiInfoVo> apiMap) {
		...
		for (String e : values.values()) {
			ZRApiSettings settings = JsonUtil.jsonToObj(e, ZRApiSettings.class);
